...
public class ConfigWindow : Window, IDisposable {
	...
	public ConfigWindow(SmartModSwitch smsw) : base(
		...
		assignmentGroups = new SelectListComponent<AssignmentGroup>(smsw.Config.AssignmentGroups, (entry, index) => {
			smsw.Logger.Info($"Selected: {entry}");
		}, () => {
			return new AssignmentGroup("new");
		}, (entry, index, isSelected) => {
			return ImGui.Selectable($"{index}: {entry.Name}", isSelected);
		});
	}
